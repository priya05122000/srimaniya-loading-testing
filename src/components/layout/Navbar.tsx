...
const Navbar = ({ sticky = true }: NavbarProps) => {
  ...
  return (
    <nav
      className={`navbar h-20 z-9999 bg-(--blue) w-full shadow-sm border-b border-(--grey-custom) ${sticky ? "fixed top-0 " : ""} transition-opacity duration-500 navbar--visible`}
    >
      <div className="grid grid-cols-[1fr_1.5fr] sm:grid-cols-[1.5fr_1fr] lg:grid-cols-[2fr_1fr] xl:grid-cols-[3fr_1fr] h-full">
        ...
        <div className="flex items-center order-1 pl-6 sm:pl-8 sm:border-r border-(--grey-custom) ">
          ...
          {/* Nav Links */}
          <div
            className={`nav-elements fixed top-20 left-0 h-[calc(100vh-80px)] w-70 bg-(--blue) transition-transform duration-300 z-40 ${menuOpen ? "translate-x-0 px-6 py-8 shadow-lg" : "-translate-x-full px-6 py-8"} xl:static xl:h-auto xl:w-auto xl:bg-transparent xl:translate-x-0 xl:px-0 xl:py-0 xl:shadow-none xl:flex xl:items-center`}
          >
              ...
              {NAV_LINKS.map((link) => (
                <li key={link.name} className="relative group">
                  <Link
                    href={link.href ?? "#"}
                    hrefLang="en"
                    className={`text-base text-(--white-custom) transition-colors duration-200 relative py-2 ${isActive(link.href ?? "#") ? "border-b border-(--white-custom)" : "hover:border-b hover:border-(--white-custom)"}`}
            ...
            <div className="mt-6 xl:mt-0 xl:ml-4">
              <button
                className="relative flex justify-center items-center gap-1 rounded-full bg-(--blue) overflow-hidden cursor-pointer border border-(--yellow) group transition-all duration-300 px-3 py-1 min-w-27.5"
                onClick={() => setShowBrochureModal(true)}
              >
                <Paragraph
                  size="base"
                  className="relative gap-x-1 z-20 flex items-center text-center no-underline w-full text-(--yellow) transition-all duration-300 group-hover:text-(--blue)"
                >
                  Brochure <Download aria-label="Download brochure" />
                </Paragraph>
        ...
        >
          <Image
            ...
            width={500}
            height={500}
            className="h-16 sm:h-12 w-auto object-contain image-tag"
            priority
          />
        </Link>
      </div>
