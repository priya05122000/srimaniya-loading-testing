    ...
    >
      ...
      <div className="absolute bottom-0 left-0 z-0 pointer-events-none">
        <Image
          ...
          width={500}
          height={500}
          className="w-auto h-full sm:h-100 object-contain image-tag"
          priority
        />
      </div>
      {/* Main content above */}
              ...
              <div className="grid grid-cols-4 sm:grid-cols-4  w-full">
                {SOCIALS.map((item) => (
                  <Link
                    key={item.label}
                    href={item.href}
                    className={`flex items-center  gap-6 py-6 text-(--white-custom) transition  group sm:border-r last:border-0 border-(--black-custom) ${
