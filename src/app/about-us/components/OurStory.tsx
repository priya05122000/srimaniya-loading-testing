...
const STORY_TEXT = (
  <>
    <span className="text-base lg:text-lg font-inter font-semibold block mb-2">
      Our Story: The Journey of a Young Driver Who Redefined Hospitality
    </span>
    <br />
    ...
    <br />
    <span className="text-base lg:text-lg font-inter font-semibold block mb-2">
      The Legacy Continues: Educating the Next Generation
    </span>
    <br />
...
const OurStory: React.FC = () => {
  ...
  const handleReadMore = () => {
    if (paragraphRef.current) {
      // Scroll down by one line height (approx)
      const lineHeight = parseInt(
        getComputedStyle(paragraphRef.current).lineHeight
      );
      paragraphRef.current.scrollBy({
        top: lineHeight,
        behavior: "smooth",
      });
    }
  ...
  return (
    <div ref={sectionRef}>
      <div className="grid grid-cols-1 md:grid-cols-[auto_2fr] gap-0 h-full">
        ...
        {/* Right: Content */}
        <div
          className="bg-(--blue) text-(--white-custom) flex flex-col justify-around py-10  pl-6 sm:pl-16 pr-6 sm:pr-8 space-y-8"
          data-section
        >
          <Heading
            level={6}
            className="text-(--white-custom) font-bold"
          >
            Our Story
          </Heading>
          <Paragraph
