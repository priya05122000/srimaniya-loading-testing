    ...
    if (type === "checkbox") {
      ...
      if (file) {
        if (!allowedResumeTypes.includes(file.type)) {
          ...
          if (fileInputRef.current) fileInputRef.current.value = ""; // reset visually
          return;
        }
      }
    ...
    >
      <div className="h-full relative">
        ...
        <div className="md:absolute md:right-0 md:top-0 w-full md:w-1/2">
          <div
            className="h-full sm:h-[600px] bg-(--blue-overlay-light) backdrop-blur-md p-6 sm:p-8 flex flex-col justify-between gap-6"
            data-section
          >
              ...
              <div className="block text-end mt-2">
                <div className="flex justify-end ml-auto">
                  ...
                  >
                    <span
                      className={`${
                        localLoading ? "disabled" : ""
                      } relative z-20 text-center no-underline w-full px-2 py-1 text-(--yellow) text-base transition-all duration-300 group-hover:text-(--blue)`}
                    >
